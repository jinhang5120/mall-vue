  ...
  methods: {
      ...
      while (catId != 0) {
        for (let i = 0; i < this.categoryArray.length; i++) {
          if (this.categoryArray[i].catId == catId) {
            //从头部插入数组
            this.catelogPath.unshift(catId);
            catId = this.categoryArray[i].parentCid;
            break;
          }
        }
      }
      console.log("this.catelogPath:", this.catelogPath);
    },
    getCategoryArray() {
      this.$http({
    ...
    getCategorys() {
      this.$http({
        ...
        this.categorys = data.pmsCategoryEntities;
      });
    },
    init(id) {
        ...
        if (this.dataForm.attrGroupId) {
          this.$http({
            ...
            if (data && data.code === 0) {
              ...
              this.catelogIdThirdLevel = data.attrGroup.catelogId;
              //查出catelogId的完整路径
