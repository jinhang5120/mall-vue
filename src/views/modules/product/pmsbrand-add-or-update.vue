        ...
        firstLetter: [
          {
            validator: (rule, value, callback) => {
              if (value === null) {
                callback(new Error("首字母要填写"));
              } else if (!/^[a-zA-Z]/.test(value)) {
                callback(new Error("首字母范围a-z或A-Z"));
              } else {
                callback();
              }
            },
        ...
        sort: [
          {
            validator: (rule, value, callback) => {
              if (value === "") {
                callback(new Error("排序字段要填写"));
              } else if (!Number.isInteger(value) || value < 0) {
                callback(new Error("排序树必须大于等于0"));
              } else {
                callback();
              }
            },
    ...
    dataFormSubmit() {
      this.$refs["dataForm"].validate((valid) => {
        if (valid) {
          this.$http({
            url: this.$http.adornUrl(
              `/product/pmsbrand/${!this.dataForm.brandId ? "save" : "update"}`
            ),
            method: "post",
